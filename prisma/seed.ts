...
function metricsForSport(slug: string): Record<string, string | number> {
  ...
  switch (slug) {
    case "baseball": {
      ...
      if (pitcher) { m.fbVelo = rint(84, 96); m.era = rfloat(1.8, 5.2); m.whip = rfloat(0.9, 1.7); m.inningsPitched = rint(20, 85); m.strikeouts = rint(25, 105); }
      else { m.exitVelo = rint(88, 104); m.sixtyTime = rfloat(6.3, 7.4, 2); m.avg = rfloat(0.24, 0.41, 3); m.obp = rfloat(0.31, 0.47, 3); m.slg = rfloat(0.35, 0.69, 3); m.ops = parseFloat(((m.obp as number) + (m.slg as number)).toFixed(3)); m.strikeouts = rint(15, 45); }
      break;
    }
    case "softball": {
      const pitcher = chance(0.4);
      m.batsThrows = pick(HAND);
      if (pitcher) { m.pitchVelo = rint(58, 70); m.era = rfloat(1.4, 4.6); m.whip = rfloat(0.9, 1.6); m.strikeouts = rint(40, 180); }
      else { m.exitVelo = rint(66, 80); m.homeToFirst = rfloat(2.7, 3.4); m.avg = rfloat(0.27, 0.44, 3); m.obp = rfloat(0.34, 0.5, 3); m.slg = rfloat(0.38, 0.72, 3); }
      break;
    }
    case "mens-soccer":
    case "womens-soccer": {
      const gk = chance(0.15);
      m.minutes = rint(400, 1700); m.starts = rint(4, 20);
      if (gk) { m.saves = rint(30, 95); m.cleanSheets = rint(2, 11); }
      else { m.goals = rint(0, 18); m.assists = rint(0, 14); }
    ...
    case "womens-basketball": {
      m.ppg = rfloat(4, 24, 1); m.rpg = rfloat(2, 12, 1); m.apg = rfloat(0.8, 8, 1); m.spg = rfloat(0.3, 3, 1); m.bpg = rfloat(0.1, 2.6, 1); m.fgPct = rfloat(38, 58, 1); m.threePct = rfloat(26, 44, 1); m.ftPct = rfloat(60, 90, 1);
      break;
    }
    case "womens-volleyball": {
      m.approachTouch = `${rint(9, 10)}'${rint(0, 11)}"`; m.killsPerSet = rfloat(1.5, 4.6, 1); m.hittingPct = rfloat(0.18, 0.42, 3); m.blocksPerSet = rfloat(0.4, 1.9, 1); m.digsPerSet = rfloat(1.5, 5.5, 1);
      break;
    }
    case "football": {
      m.fortyTime = rfloat(4.42, 5.3); m.vertical = rint(26, 40); m.bench = rint(10, 30);
      if (chance(0.5)) { m.touchdowns = rint(1, 18); if (chance(0.5)) m.rushYards = rint(300, 1400); else m.recYards = rint(250, 1200); }
      else { m.tackles = rint(20, 110); m.sacks = rint(0, 12); m.interceptions = rint(0, 6); }
      break;
    }
    case "mens-golf":
    case "womens-golf": {
      m.scoringAvg = rfloat(70.5, 79.5, 1); m.handicap = rfloat(0, 6, 1); m.lowRound = rint(66, 74); m.topFinishes = rint(0, 8);
      break;
    }
    ...
    case "womens-track-field": {
      ...
      m.primaryEvent = ev;
      m.personalRecord = ev.includes("m") ? `${rint(10, 52)}.${rint(10, 99)}` : `${rfloat(1.7, 7.6)} m`;
      if (chance(0.5)) m.meetResult = pick(["Conf. Champion", "2nd Region", "Nationals Qualifier", "5th Conf."]);
      break;
    }
    case "mens-cross-country": { m.pr8k = `${rint(24, 27)}:${rint(10, 59)}`; m.pr5k = `${rint(14, 16)}:${rint(10, 59)}`; break; }
    case "womens-cross-country": { m.pr6k = `${rint(20, 23)}:${rint(10, 59)}`; m.pr5k = `${rint(17, 19)}:${rint(10, 59)}`; break; }
    case "mens-swimming":
    case "womens-swimming": {
      const ev = pick(["50 Free", "100 Free", "100 Back", "200 IM", "100 Fly", "500 Free"]);
      m.primaryEvent = ev; m.bestTime = `${rint(0, 4)}:${rint(10, 59)}.${rint(10, 99)}`; m.courseType = pick(["SCY", "LCM"]);
      break;
    }
    case "wrestling": { m.weightClass = pick(["125", "133", "141", "149", "157", "165", "174", "184", "197", "285"]); m.record = `${rint(12, 35)}-${rint(2, 14)}`; m.pins = rint(2, 18); break; }
    case "tennis": { m.utr = rfloat(7, 13, 1); m.singlesRecord = `${rint(8, 24)}-${rint(2, 12)}`; m.doublesRecord = `${rint(6, 20)}-${rint(2, 10)}`; m.handedness = pick(["Right", "Left"]); break; }
  }
  return m;
}

const PER_SPORT: Record<string, number> = {
...
/** Idempotent: wipes and repopulates the database. Safe to call from a route. */
export async function seedDatabase(prisma: PrismaClient): Promise<SeedResult> {
  seed = 20270918; // reset RNG for reproducibility
  ...
  for (const s of SPORTS) {
    const created = await prisma.sport.create({
      data: {
        slug: s.slug, name: s.name, gender: s.gender, order: s.order,
        fields: {
          create: s.fields.map((f, i) => ({
            key: f.key, label: f.label, group: f.group, type: f.type,
            unit: f.unit ?? null, options: f.options ? JSON.stringify(f.options) : null, order: i,
          })),
        },
      },
    });
    sportIds[s.slug] = created.id;
  }

  // schools
  const schoolData = SCHOOLS.map((sc) => ({
    slug: slugify(sc.name), name: sc.name, city: sc.city, state: sc.state, conference: sc.conference,
  }));
  await prisma.school.createMany({ data: schoolData });
  const schools = await prisma.school.findMany();
  const schoolBySlug = Object.fromEntries(schools.map((s) => [s.slug, s]));

  // users + demo coach
  const pwHash = await bcrypt.hash("password123", 10);
  await prisma.user.create({ data: { email: "admin@jucoportal.com", passwordHash: pwHash, name: "Portal Admin", role: "ADMIN" } });
  const demoSchool = schoolBySlug[slugify(SCHOOLS[0].name)];
  const coachUser = await prisma.user.create({ data: { email: "coach@jucoportal.com", passwordHash: pwHash, name: "Coach Dan Rivera", role: "COACH" } });
  const demoCoach = await prisma.coach.create({
    data: { userId: coachUser.id, schoolId: demoSchool.id, name: "Coach Dan Rivera", title: "Head Baseball Coach", email: "coach@jucoportal.com", phone: "(760) 555-0142", verified: true },
  });
  const athleteUser = await prisma.user.create({ data: { email: "athlete@jucoportal.com", passwordHash: pwHash, name: "Jordan Athlete", role: "ATHLETE" } });

  // build athletes in memory, then batch insert
  const usedSlugs = new Set<string>();
  const rows: Prisma.AthleteCreateManyInput[] = [];

  for (const sport of SPORTS) {
    const n = PER_SPORT[sport.slug] ?? 15;
    for (let i = 0; i < n; i++) {
      ...
      rows.push({
        slug, firstName: first, lastName: last,
        sportId: sportIds[sport.slug], schoolId: school.id, schoolName: school.name,
        positions: pick(sport.positions), classYear, transferYear,
        transferSemester: pick(["Fall", "Spring"]), transferStatus: pick(STATUSES),
        eligibilityYears: classYear === "Freshman" ? rint(2, 3) : rint(1, 2),
        heightInches: inches, weightLbs: weightForHeight(inches, sport.slug),
        handedness: sport.slug === "baseball" || sport.slug === "softball" ? pick(["R/R", "L/L", "L/R", "S/R"]) : null,
        city: school.city, state: school.state,
        gpa: rfloat(2.4, 4.0), major: pick(MAJORS), creditsCompleted: rint(24, 62),
        ...
        showEmail: chance(0.55), showPhone: chance(0.3),
        coachName: `Coach ${pick(LAST)}`, coachTitle: pick(COACH_TITLES),
        coachEmail: `${sport.slug.split("-")[0]}coach@${school.slug.replace(/-/g, "")}.edu`,
        coachPhone: `(${rint(200, 989)}) 555-${rint(1000, 9999)}`,
        verification: pick(VERIFICATIONS), published: true, approved: true, views: rint(0, 480),
      });
    }
  }

  await prisma.athlete.createMany({ data: rows });
  ...
  await prisma.rosterUpload.update({ where: { id: roster.id }, data: { rowsTotal: mine.count, rowsReady: mine.count } });

  // claim one profile for the demo athlete account
  const claimTarget = await prisma.athlete.findFirst({ orderBy: { slug: "asc" } });
  if (claimTarget) {
  ...
  return { sports: SPORTS.length, schools: schools.length, athletes: rows.length, coachRoster: mine.count };
}
...
async function main() {
  ...
  console.log(`✅ ${r.athletes} athletes • ${r.sports} sports • ${r.schools} schools • ${r.coachRoster} in demo coach roster`);
  console.log("   Demo logins (password: password123):");
  console.log("     admin@jucoportal.com | coach@jucoportal.com | athlete@jucoportal.com");
  await prisma.$disconnect();
}

// run only when invoked directly (not when imported by the seed route)
